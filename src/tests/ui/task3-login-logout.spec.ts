...
test.describe('Task 3 — Login and logout', () => {
  test('Logs in with stored credentials and logs out cleanly', { tag: '@e2e' }, async ({
    navigationPage,
    loginPage,
  }) => {
    test.skip(
  ...
  test('Shows error for unknown email with wrong password', { tag: '@negative' }, async ({
    navigationPage,
    loginPage,
  }) => {
    await navigationPage.gotoPath('/');
    await navigationPage.goToSignupLogin();
    await loginPage.loginWithCredentials(`ghost_${Date.now()}@example.com`, WRONG_PASSWORD_PLACEHOLDER);
